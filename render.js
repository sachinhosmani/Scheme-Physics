...
g_Box2D.applyForce = function (id, force, time) {
    var f_body = g_Box2D.bodies[g_Box2D.findBodyIndex(id)];
    if (f_body === undefined) {
        return;
    }
    if (force === undefined) {
    ...
    // push the new force
    f_body.forces.force_list.push(force);
    // force has to act for time more milliseconds
    f_body.forces.time_left.push(time);
    // push the body for force application (during world steps)
    g_Box2D.force_bodies.push(f_body);
};
g_Box2D.getBodyList = function () {
    // get a list of bodies' ids
    var body_list = [];
    for (var i = 0; i < g_Box2D.body_count; i++)
...
/* Types*/
g_Box2D.circleShape = function (radius) {
...
g_Box2D.rectangleShape = function (width, height) {
    // rectangle shape
    var shape = {
...
g_Box2D.polygonShape = function (vertices) {
    // polygon shape
    ...
    return {
        ...
        angle: function (vertex) {
            ...
            var to2PI = function (p_angle) {
                if (p_angle < 0) {
                    // so that angle lies in [0, 2 * PI)
                    return 2 * Math.PI + p_angle;
                }
                return p_angle;
            };
            if (vertex !== undefined) {
...
g_Box2D.attributes = function (density, friction, restitution) {
    // holds attributes of a body
    return {
...
g_Box2D.createBody = function (p_type, p_shape, p_position, p_angle, p_velocity, p_attributes, p_color, p_id) { //body container
    var body = {
        ...
        contains: function (pos) {
            ...
            case "Polygon":
                ...
                // pole vertex
                var pole = g_Helper.midPoint(vertices);
    ...
    if (body.shape.type === "Circle") {
        ...
        body.axisShape = g_Box2D.polygonShape([g_Box2D.vector2D(0, 0), g_Box2D.vector2D(body.shape.radius * Math.cos(body.angle), body.shape.radius * Math.sin(body.angle)), g_Box2D.vector2D(body.shape.radius * Math.cos(body.angle + 0.1), body.shape.radius * Math.sin(body.angle + 0.1))]);
    }
    return body;
}
...
g_WebGL.tick = function () {
    ...
    for (i = 0; i < g_Box2D.body_count; i++) {
        ...
        if (g_Box2D.bodies[i].axisShape !== undefined) {
            ...
            g_WebGL.setColor(g_Box2D.color(0, 0, 0));
